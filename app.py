...
wifi_annual_opex = wifi_capex * wifi_maint

wifi_total = (wifi_capex + wifi_annual_opex * years) * \
             sla_multiplier(sla) * growth_multiplier(growth, years)
...
p5g_annual_opex = p5g_capex * p5g_maint

p5g_total = (p5g_capex + p5g_annual_opex * years) * \
            sla_multiplier(sla) * growth_multiplier(growth, years)
...
for y in range(1, years+1):
    wifi_y = wifi_capex + wifi_annual_opex * y
    p5g_y = p5g_capex + p5g_annual_opex * y
    if p5g_y <= wifi_y:
        break_even_year = y
        break
...
if break_even_year:
    ...
    st.info("No break-even within selected horizon.")
...
cash_flows_wifi = [-wifi_capex] + [-wifi_annual_opex] * years
cash_flows_p5g = [-p5g_capex] + [-p5g_annual_opex] * years

npv_wifi = npf.npv(discount_rate, cash_flows_wifi)
npv_p5g = npf.npv(discount_rate, cash_flows_p5g)
...
st.write(f"ROI difference (Wi-Fi vs 5G): {roi:.1f}%")

# ============================================================
# SENSITIVITY HEATMAP
# ============================================================

st.markdown('<div class="section-title">4️⃣ Sensitivity Heatmap</div>', unsafe_allow_html=True)

growth_range = np.linspace(0, 20, 6)
maint_range = np.linspace(0.1, 0.25, 6)
...
alt_discount = st.slider("Alternate Discount Rate (%)", 5, 20, 12) / 100

npv_wifi_alt = npf.npv(alt_discount, cash_flows_wifi)
npv_p5g_alt = npf.npv(alt_discount, cash_flows_p5g)

st.write(f"At {alt_discount*100:.0f}% discount rate:")
